...
gpii.test.integration.enableRegisteredATKey = function (options, solutionsId, fromSet) {
    if (fromSet) {
        return fluid.get(options, "registryName");
    } else {
...
fluid.defaults("gpii.test.integration.mockSettingsHandlerRegistry.windows", {
    ...
    settingsHandlers: {
        ...
        "gpii.windows.enableRegisteredAT": {
            optionsPathKey: "registryName" //gpii.test.integration.enableRegisteredATKey
        }
    }
});
...
gpii.test.integration.enableRegisteredAT.getImpl = function (that, payload, solutionId) {
    var isRunning = gpii.processReporter.handleIsRunning(payload.options);
    var response = { "running": isRunning };
    var togo = gpii.test.integration.mockSettingsHandler.returnAsyncResponse(response, that.options.asyncGet);
    return togo;
};
...
gpii.test.integration.enableRegisteredAT.setImpl = function (that, payload, solutionId) {
    ...
    var procInfo = gpii.test.integration.generateMockProcInfo(command);
    var currentState;
    debugger;
    if (enable) {
        currentState = gpii.test.integration.startWindowsRegisteredAT(processesBridge, procInfo);
    } else {
        currentState = gpii.test.integration.stopWindowsRegisteredAT(processesBridge, procInfo);
    }
    var togo = {
        running: {
    ...
    debugger;
    return gpii.test.integration.mockSettingsHandler.returnAsyncResponse(togo, that.options.async);
};
...
/**
 * "Start" a Windows AT, such as the magnifier.  Simulated by adding the
 * mock process to the list of processes known to the given proceesesBridge, if
 * not already present in the list.  Returns whether there was a process already
 * running.
 *
 ...
 * @return {Boolean}
 */
gpii.test.integration.startWindowsRegisteredAT = function (processesBridge, mockProcInfo) {
    var togo;
...
/**
 * "Stop" Windows AT if running.  Simulated by removing the mock process from
 ...
 * @return {Boolean}
 */
gpii.test.integration.stopWindowsRegisteredAT = function (processesBridge, mockProcInfo) {
    var togo;
    var procInfo = processesBridge.findFirstProcessByCommand(mockProcInfo.command);
 ...
 */
gpii.test.integration.generateMockProcInfo = function (command) {
    return {
        command: command,
