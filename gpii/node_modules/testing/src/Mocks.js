...
gpii.test.integration.resolveHandlerKey = function (that, payload, solutionId) {
    ...
    if (typeof(pathKey) === "string") {
        console.log("    optionsPathKey: '" + pathKey + "', '" + fluid.get(payload.options, pathKey) + "'");
        //debugger;
        return fluid.get(payload.options, pathKey);
    } else if (typeof(pathKey) === "function") {
        var pathKeyVal = pathKey(payload.options, solutionId);
        console.log("    optionsPathKey: '" + pathKey + "', { pathKeyVal: " + pathKeyVal + ", solutionId: " + solutionId + "}");
        //debugger;
...
gpii.test.integration.mockSettingsHandler.getImpl = function (that, payload, solutionId) {
    ...
    console.log("MOCK GET IMPL: key = " + key + ", store = '" + fluid.prettyPrintJSON(store) + ", response = " + fluid.prettyPrintJSON(response));
     ;
...
gpii.test.integration.mockSettingsHandler.get = function (that, payload) {
    var key = that.options.optionsPathKey;
    if (key && (typeof key == 'string') && key.indexOf("flexible") !== -1) {
        debugger;
    }
    console.log("gpii.test.integration.mockSettingsHandler.get(), that = [" + that.typeName + ", " + that.options.optionsPathKey + "]");
...
gpii.test.integration.enableRegisteredATKey = function (options) {
...
gpii.test.integration.mockSettingsHandlerRegistry.populateOne = function (that, mock, key) {
    ...
    fluid.setGlobalValue(that.options.rootPath + "." + key + ".get", mock.get);  // THIS IS IT!!!
    console.log("that isa '" + that.typeName + "', key = '" + key + "'");
};
...
gpii.test.integration.mockSettingsHandlerRegistry.populate = function (that) {
    fluid.each(that.options.settingsHandlers, function (options, key) {
        var mock = gpii.test.integration.mockSettingsHandler(options);  // LOOK HERE
...
//            optionsPathKey: gpii.test.integration.enableRegisteredATKey
        }
    }
});
...
gpii.test.integration.mockMagnify = fluid.freezeRecursive({
    command: "Magnify.exe",
    pid: 5012,
    ppid: 968,
    uid: 501,
...
gpii.test.integration.mockProcessesList = [
    gpii.test.integration.mockMagnify,
...
// Check if the given setting is set.  Returns a boolean indicating whether the
// setting is set.
gpii.processReporter.checkSetting = function (schema, setting, value) {
    debugger;
    var x = 5;
/* This should use the mockSettingsHandler, instead of the actual linux settings handler (gsettings)
